...
function QuestionModal({ question, onClose, onSaved, onError }) {
  ...
  async function sendAnswer() {
    ...
    try {
      ...
      // ✅ your backend: PATCH /api/questions/{questionId}/answer
      // DTO: QuestionAnswerRequest (we assume it has field like "answerMessage" or "answer")
      // We'll send BOTH keys to be safe; backend will bind the correct one.
          ...
          <div className="d-flex justify-content-between align-items-center gap-2 flex-wrap">
            <div>
              ...
              <div className="text-muted small">
                Status:{" "}
                <span className="fw-semibold">{answered ? "ANSWERED" : "PENDING"}</span>
              </div>
            </div>
...
export default function AdminQuestions() {
  ...
  // optional: load full question before open modal (if you want)
  async function openQuestion(q) {
    // if you don’t need GET ONE, just setSelected(q)
    try {
      setLoading(true);
      ...
      if (!res.ok) {
        // fallback: open the partial item
        setSelected(q);
        return;
      }
