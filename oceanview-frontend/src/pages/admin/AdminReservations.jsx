...
export default function AdminReservations() {
  ...
  const [roomFilter, setRoomFilter] = useState("ALL"); // Standard/Deluxe/Suite
  const [fromDate, setFromDate] = useState(""); // createdAt filter (client-side)
  ...
  async function updateStatus(reservation, status) {
    try {
      ...
      if (!res.ok) {
        setAlert({ type: "error", msg: data?.message || data?.error || "Failed to update status." });
        return;
      }
  ...
  return (
    <div className="container-fluid py-4">
      ...
      <div className="d-flex flex-wrap justify-content-between align-items-center gap-2">
        <div>
          <h2 className="ov-h2 mb-1">Confirm Reservations</h2>
        </div>
              ...
              {loading && items.length === 0 ? (
                ...
                filtered.map((r, idx) => (
                  <tr key={r.id || r.reservationNo}>
                    ...
                    <td>{r.roomType}</td>
                    <td>
                      <div>{fmtDateOnly(r.checkInDate)} → {fmtDateOnly(r.checkOutDate)}</div>
                      <div className="text-muted small">
                    ...
                    <td className="text-end">
                      <div className="d-flex justify-content-end gap-2 flex-wrap">
                        <button className="btn ov-btn-soft ov-btn-xs" onClick={() => openEdit(r)}>
