...
export default function StaffProfile() {
  ...
  async function loadProfile() {
    try {
      ...
      if (!res.ok) {
        setAlert({ type: "error", msg: data?.message || data?.error || "Failed to load profile." });
        return;
      }
  ...
  async function saveProfile() {
    ...
    try {
      ...
      if (!res.ok) {
        setAlert({ type: "error", msg: data?.message || data?.error || "Failed to update profile." });
        return;
      }
      ...
      // ✅ same behavior as AdminProfile (optional)
      setTimeout(() => {
